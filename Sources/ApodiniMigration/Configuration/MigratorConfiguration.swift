...
public enum ResourceLocation {
    ...
    /// - Throws: If resource not found or if decoding fails
    func instance<D: Decodable>() throws -> D {
        ...
        case let .resource(bundle, fileName, format):
            guard let resourcePath = bundle.path(forResource: fileName, ofType: format.rawValue) else {
                throw ResourceLocationError.notFound(message: "Resource \(fileName).\(format.rawValue) not found in the bundle")
            }
            path = .init(resourcePath)
        }
...
public enum MigrationGuideConfiguration {
    ...
    case compare(_ documentLocation: ResourceLocation, exportAt: ExportPath, format: FileFormat)
    /// Reads the migration guide from `migrationGuideLocation` and exports it at `exportAt` as the specified file format`
    case read(_ migrationGuideLocation: ResourceLocation, exportAt: ExportPath, format: FileFormat)
}
