...
struct DeployWebServiceCommand: ParsableCommand {
    ...
    @Option(help: "Defines the AWS API Gateway ID that is used. If no value is passed in, the Deployment Provider creates a new AWS API Gateway")
...
struct LambdaDeploymentProviderImpl: DeploymentProvider {
    ...
    private var tmpDirName: String { "ApodiniDeployerTmp" }
    ...
    private func retrieveDeployedSystem(usingDockerImage dockerImageName: String) throws -> LambdaDeployedSystem {
        ...
        try runInDocker(
            ...
            bashCommand: "swift run \(productName) \(flattenedWebServiceArguments) deploy export-ws-structure aws \(filePath) --identifier \(Self.identifier.rawValue) --aws-api-gateway-api-id \(awsApiGatewayApiId) --aws-region \(awsRegion)"
        )
