...
public enum WellKnownEnvironmentVariables {
    ...
    /// Key for an environment variable specifying the execution mode of ApodiniDeployer, ether dump the WebService's model structure or launch the WebService with custom config
    public static let executionMode = "AD_EXECUTION_MODE"
    
    /// Key for an environment variable specifying the url of the directory used for ApodiniDeployer, either the outputURL or configURL
