...
public struct DownloadConfiguration {
    ...
    internal func retrieveFileInfo(_ fileName: String, in app: Application) throws -> FileInfo? {
        let fileManager = FileManager.default
        let searchableDirectories: [String] = directories.map { $0.path(for: app) }
    ...
    internal func retrieveFileInfos(_ fileName: String, in app: Application) throws -> [FileInfo] {
        let fileManager = FileManager.default
        let searchableDirectories: [String] = directories.map { $0.path(for: app) }
        var infos: [FileInfo] = []
