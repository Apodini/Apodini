...
struct ComponentModifierComponent<V: ComponentModifier>: Component {
    init<C: Component>(_ component: C) {
        fatalError("(Associated with Question 1.1): Not implemented. And not sure how we could do that ...? I guess I can use it only to store the type information and provide some constraints on what can be done in a ComponentModifier?")
    }
}
...
#warning("Question 1.2: Is it a general workflow in SwiftUI to use a ModifiedView struct to encapsulate Views and Modifier in the view tree?")
