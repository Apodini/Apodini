...
struct WebServiceStructure {
    ...
    mutating private func serviceParameters(from endpointParameters: [AnyEndpointParameter]) -> [ServiceParameter] {
        return endpointParameters.map { parameter in
            let reference = schemaBuilder.build(for: parameter.propertyType, root: false) ?? .empty
            return .init(
                name: parameter.name,
                necessity: parameter.necessity,
                type: parameter.parameterType,
                nilIsValidValue: parameter.nilIsValidValue,
