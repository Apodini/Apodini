...
extension RemoteHandlerInvocationManager {
    ...
    ) async throws -> H.Response.Content where H.ArgumentsStorage == InvocableHandlerEmptyArgumentsStorage<H> {
        try await invokeImp(handlerType: handlerType, handlerId: handlerId, collectedInputArgs: arguments).get()
    }
    ...
    private func invokeImp<H: InvocableHandler>(
        ...
        guard let internalInterfaceExporter = self.app.storage.get(ApodiniDeployerInterfaceExporter.StorageKey.self) else {
            return eventLoop.makeFailedFuture(ApodiniDeployerError(message: "Unable to get \(ApodiniDeployerInterfaceExporter.self) object"))
        }
        
        guard let targetEndpoint: Endpoint<H> = internalInterfaceExporter.getEndpoint(withIdentifier: handlerId, ofType: H.self) else {
            return eventLoop.makeFailedFuture(ApodiniDeployerError(message: "Unable to find target endpoint. (handlerId: '\(handlerId)', handlerType: '\(H.self)')"))
        }
    ...
    private func invokeRemotely<H: InvocableHandler>( // swiftlint:disable:this function_body_length cyclomatic_complexity
        ...
        guard let runtime = internalInterfaceExporter.deploymentProviderRuntime else {
            return eventLoop.makeFailedFuture(ApodiniDeployerError(message: "Unable to find runtime"))
        }
            ...
            case .invokeDefault(let url):
                ...
                .flatMapThrowing { (response: HTTPClient.Response) -> H.Response.Content in
                    guard var responseBody = response.body else {
                        throw ApodiniDeployerError(message: "Unexpectly got empty response")
                    }
                    guard let handlerResponse = try responseBody.readJSONDecodable(
                        ...
                        throw ApodiniDeployerError(message: "Unable to decode response body")
                    }
    ...
    func invokeImp(
        ...
        return responseFuture.flatMapThrowing { (response: Apodini.Response<H.Response.Content>) -> H.Response.Content in
            guard response.connectionEffect == .close else {
                throw ApodiniDeployerError(message: "Unexpected response value: \(response). Expected '.final'.")
            }
            guard let content = response.content else {
                throw ApodiniDeployerError(message: "Unable to get response content")
            }
            return content
        }
    }
