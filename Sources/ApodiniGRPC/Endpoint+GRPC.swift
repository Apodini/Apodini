...
public func gRPCServiceName<H: Handler>(from endpoint: Endpoint<H>) -> String {
    if let serviceName = endpoint.content[Context.self].get(valueFor: GRPCServiceNameContextKey.self) {
        return serviceName
    }
...
public func gRPCMethodName<H: Handler>(from endpoint: Endpoint<H>) -> String {
    if let methodName = endpoint.content[Context.self].get(valueFor: GRPCMethodNameContextKey.self) {
        return methodName
    }
