...
extension GRPCService {
    ...
    private func drainClientStream(from request: Vapor.Request,
        ...
        request.body.drain { (bodyStream: BodyStreamResult) in
            ...
            case let .buffer(byteBuffer):
                ...
                let messages = self.getMessages(from: data, remoteAddress: request.remoteAddress)
    ...
    private func write(_ response: Apodini.Response<EnrichedContent>, to stream: BodyStreamWriter, promise: EventLoopPromise<Void>? = nil) {
        if let element = response.content?.response,
    ...
    private func handleValue(_ value: Result<Apodini.Response<EnrichedContent>, Error>, responseWriter: BodyStreamWriter, serviceStreaming: Bool) {
        ...
        case let .success(response):
            if response.connectionEffect == .close {
                ...
                // Only if service-streaming is enabled,
