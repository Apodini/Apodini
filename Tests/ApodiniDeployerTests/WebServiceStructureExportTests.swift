...
@testable import ApodiniDeployerBuildSupport
...
class WebServiceStructureExportTests: ApodiniDeployerTestCase {
    func testExportLambdaDeployedSystem() throws { // swiftlint:disable:this function_body_length
        let (_, deployedSystem) = try StaticDeploymentProvider(
            executableUrl: Self.ApodiniDeployerTestWebServiceTargetUrl
        )
        .retrieveSystemStructure(
            Self.ApodiniDeployerTestWebServiceTargetUrl,
            providerCommand: "aws",
            additionalCommands: [
                "--identifier",
