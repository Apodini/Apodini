...
class MockExporter<Request: ExporterRequest>: InterfaceExporter {
    ...
    func retrieveParameter<Type: Decodable>(_ parameter: EndpointParameter<Type>, for request: Request) throws -> Type? {
        ...
        if first == nil {
            fatalError("MockExporter failed to retrieve next parameter. Queue is empty")
        }
        parameterValues.removeFirst()

        guard let casted = first as? Type? else {
            fatalError("MockExporter: Could not cast parameter \(String(describing: first)) to type \(Type?.self)")
        }
        return casted
    }
}
