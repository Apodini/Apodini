...
class RESTExporterConfigurationTests: XCTApodiniHTTPTest {
    ...
    func testRESTRequestWithDefaultEncoderConfig() throws {
        ...
        try XCTHTTPCheck(testService, configuration: REST()) {
            HTTPCheck(HTTPRequest(path: "v1/user/\(userId)?name=\(name)")) { response in
                XCTAssertEqual(response.status, .ok)
                let container = try response.decodeBody(ResponseContainer<User>.self)
    ...
    func testRESTRequestWithWithJSONEncoderConfig() throws {
        ...
        try XCTHTTPCheck(testService, configuration: REST(encoder: JSONEncoder(), decoder: JSONDecoder())) {
            HTTPCheck(HTTPRequest(path: "v1/user/\(userId)?name=\(name)")) { response in
                XCTAssertEqual(response.status, .ok)
                let container = try response.decodeBody(ResponseContainer<User>.self)
                XCTAssertEqual(container.data.id, userId)
